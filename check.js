...
const CLEAN_BUILD_KEY = 'sdcoir_clean_final_20260428_v2';
...
function getStockStatus(product){ const stock=Number(product.stock||0); if(stock<=0) return {label:'Out of stock', cls:'low'}; if(stock<100) return {label:'Low stock', cls:'low'}; return {label:'Available', cls:'ok'}; }
function setInventorySearch(value){ inventoryQuery=(value||'').trim().toLowerCase(); renderInventory(); }
function setInventoryFilter(value){ inventoryFilter=value||'all'; document.querySelectorAll('.filter-chip').forEach(x=>x.classList.remove('active')); const id=value==='low'?'filterLow':value==='available'?'filterAvailable':'filterAll'; document.getElementById(id)?.classList.add('active'); renderInventory(); }
function setInventorySort(value){ inventorySort=value||'name'; renderInventory(); }
function renderInventory(){
  const products=Object.entries(S.products);
  let rows=products.filter(([name,x])=>{
    const hay=(name+' '+(x.unit||'')).toLowerCase();
    const stock=Number(x.stock||0);
    if(inventoryQuery && !hay.includes(inventoryQuery)) return false;
    if(inventoryFilter==='low' && stock>=100) return false;
    if(inventoryFilter==='available' && stock<=0) return false;
    return true;
  });
  rows.sort((a,b)=>{
    if(inventorySort==='stockDesc') return Number(b[1].stock||0)-Number(a[1].stock||0);
    if(inventorySort==='stockAsc') return Number(a[1].stock||0)-Number(b[1].stock||0);
    return a[0].localeCompare(b[0]);
  });
  document.getElementById('invCount').textContent=products.length+' Items';
  document.getElementById('inventoryList').innerHTML=rows.length?rows.map(([n,x])=>{
    const status=getStockStatus(x);
    return `<div class="prod product-card"><div><div class="prod-name">${n}</div><div class="prod-meta">Current stock · ${x.unit}</div><div class="stock-status ${status.cls}">${status.label}</div></div><div class="stock ${n==='Coir Fiber'?'teal':''}">${formatNum(x.stock)}<div class="kpi-l">${x.unit}</div></div><div class="actions"><button class="btn" onclick="openDispatch('${n}')" ${Number(x.stock||0)<=0?'disabled':''}>Dispatch</button></div></div>`;
  }).join(''):`<div class="empty">No inventory matches this view</div>`;
}
